...
import math
import statistics_micropython as statistics
...
# constructor
#-----------------------------------------------------------------------
    def __init__(self, pin_step, pin_dir, pin_en, baudrate=115200):
        self.tmc_uart = TMC_UART(baudrate)
        self._pin_step = Pin(pin_step, Pin.OUT)
        self._pin_dir = Pin(pin_dir, Pin.OUT)
        self._pin_en = Pin(pin_en, Pin.OUT)
        self._pin_stallguard = None # for setStallguard_Callback function
...
#-----------------------------------------------------------------------
    def reverseDirection_pin(self):
        ...
        self._pin_dir.value(self._direction)
...
#-----------------------------------------------------------------------
    def setMStepResolutionRegSelect(self, en):                  
        ...
        if(en is True):                                                     # TODO: PEP8 style if
...
#-----------------------------------------------------------------------
    def setStallguard_Callback(self, pin_stallguard, threshold, my_callback, min_speed = 2000):
        ...
        self._pin_stallguard = Pin(pin_stallguard, Pin.IN, pull=Pin.PULL_DOWN)

        self._pin_stallguard.irq(handler=my_callback, trigger=Pin.IRQ_RISING) # TODO: find bouncetime=300 param
