...
#-----------------------------------------------------------------------
    def __init__(self, baudrate):
        ...
        self.communication_pause = int(500/baudrate*1000000)     # adjust per baud and hardware. Sequential reads without some delay fail.
...
#-----------------------------------------------------------------------
    def read_reg(self, reg):
        ...
        rFrame_bytes = bytes(self.rFrame)
        rtn = self.ser.write(rFrame_bytes)

        if rtn != len(self.rFrame):
            # print("TMC2209: Err in write {}".format(__), file=sys.stderr)
            print("TMC2209: Err in write read_reg", file=sys.stderr)
            return False

        time.sleep_us(self.communication_pause)  # adjust per baud and hardware. Sequential reads without some delay fail.

        rtn = self.ser.read(12)
        # print("received "+str(len(rtn))+" bytes; "+str(len(rtn)*8)+" bits")
        # print(rtn)
        # print("send: ", self.rFrame, "\nread: ", rtn)

        time.sleep_us(self.communication_pause)
...
#-----------------------------------------------------------------------
    def write_reg(self, reg, val):
        ...
        wFrame_bytes = bytes(self.wFrame)
        rtn = self.ser.write(wFrame_bytes)

        if rtn != len(self.wFrame):
            # print("TMC2209: Err in write {}".format(__), file=sys.stderr)
            print("TMC2209: Err in write write_reg", file=sys.stderr)
            return False
